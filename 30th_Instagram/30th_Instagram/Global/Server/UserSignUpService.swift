...
struct UserSignUpService {
  ...
  private func judgeSignUpStatus(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
    switch statusCode {
    case 200...201: return isValidLoginData(data: data) //success - status, message, data
    case 400...409: return isUsedPathErrData(data: data) //Not Found - 이메일에 해당하는 사용자 정보 없음
